...
const MFL_BASE = 'https://api.playmfl.com/players';
const LIMIT = 100; // API enforces "limit must be less than or equal to 100" -- confirmed 21 Sep 2026

const MFL_LISTINGS_BASE = 'https://api.playmfl.com/listings';
const LISTINGS_PAGE_SIZE = 50; // unverified on the new API -- if this gets rejected, the try/catch
// around fetchAllListings() in main() means listings just get skipped for that run, not fatal

// The new API requires a Bearer token tied to a logged-in wallet session (confirmed 23 Sep 2026 --
// requests with no Authorization header get a 403). These tokens are short-lived (observed ~2 hours
// from issue to expiry), so this has to be refreshed manually before each run -- there is no way for
// this script to log in on its own. Store the current token in the MFL_AUTH_TOKEN GitHub secret.
const MFL_AUTH_TOKEN = process.env.MFL_AUTH_TOKEN;

// Decodes a JWT's payload WITHOUT verifying its signature -- we only need the iat/exp timestamps it
// already carries in plain base64, not to validate authenticity (the API itself does that when we use it).
...
// Fails fast with a clear message if the token is already expired or too close to expiry to safely
// finish a run, rather than discovering this 30+ minutes into a fetch when a request suddenly 403s.
function checkTokenFreshness(token) {
  if (!token) {
    throw new Error('MFL_AUTH_TOKEN is not set. Copy a fresh Authorization header value from a logged-in ' +
      'browser session on playmfl.com (Network tab -> any authenticated request -> Headers -> ' +
      '"authorization") and update the MFL_AUTH_TOKEN secret before running.');
  }
  const payload = decodeJwtPayload(token);
  if (!payload || !payload.exp) {
    console.warn('Could not decode token expiry -- proceeding anyway, but freshness is unverified.');
    return;
  }
  const nowSec = Math.floor(Date.now() / 1000);
  const remainingMin = Math.floor((payload.exp - nowSec) / 60);
  const SAFETY_MARGIN_MIN = 10; // don't start a long run that's likely to expire before it finishes

  if (remainingMin <= 0) {
    throw new Error(`MFL_AUTH_TOKEN expired ${Math.abs(remainingMin)} minute(s) ago. Grab a fresh token and update the secret.`);
  }
  if (remainingMin <= SAFETY_MARGIN_MIN) {
    throw new Error(`MFL_AUTH_TOKEN only has ${remainingMin} minute(s) left -- too close to expiry to safely ` +
      `start this run. Grab a fresh token and update the secret.`);
  }
  console.log(`Token check: valid for ${remainingMin} more minute(s).`);
}
...
async function fetchAllPlayers() {
  ...
  while (true) {
    // No sorts param: unconfirmed whether the new API still accepts it (the old
    // "beforeId" param was rejected outright when guessed, so nothing here is
    // assumed without testing). Default ordering + beforePlayerId cursor was
    // directly confirmed to page correctly (21 Sep 2026), so that's all we rely on.
    let url = `${MFL_BASE}?limit=${LIMIT}`;
    if (cursor) url += `&beforePlayerId=${cursor}`;

    const res = await fetch(url, { headers: { Authorization: `Bearer ${MFL_AUTH_TOKEN}` } });
    if (!res.ok) {
      const body = await res.text().catch(() => '(could not read response body)');
      throw new Error(`MFL API error: ${res.status} -- ${body}`);
    }
    ...
    // NOTE: page size dropped from 1500 to 100 (API-enforced max), so this now needs
    // ~15x more requests to cover the same player base. Kept the delay short (300ms)
    // to avoid a multi-hour run, but this is a GUESS at what the new API tolerates --
    // watch the first real run for 429s/403s. If it gets rate-limited, raise this.
...
async function fetchAllListings() {
  ...
  while (true) {
    ...
    let res;
    try {
      res = await fetch(url, { headers: { Authorization: `Bearer ${MFL_AUTH_TOKEN}` } });
    } catch (e) {
...
async function main() {
  checkTokenFreshness(MFL_AUTH_TOKEN);
